...
1) Protect sensitive complaint data

2) Prevent cross-tenant access (Bank vs Airline vs Telecom)

3) Enforce security even if the UI is changed

4) Use Firebase features properly

## Considered options

1) Frontend-only security (hiding pages using React routing)

2) Database-level security using Firestore Security Rules

3) Backend-controlled security via a custom API server
